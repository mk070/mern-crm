...
class InvoiceForm extends React.Component {
  ...
  handleRowDel(item) {
    this.setState(prevState => ({
      items: prevState.items.filter(it => it !== item)
    }));
  }
  handleAddEvent() {
  ...
  handleCalculateTotal() {
    ...
    let subTotal = items.reduce((sum, item) => sum + (parseFloat(item.price) * item.quantity), 0);
    
    let taxAmmount = parseFloat(subTotal * (taxRate / 100)).toFixed(2);
    let discountAmmount = parseFloat(subTotal * (discountRate / 100)).toFixed(2);
